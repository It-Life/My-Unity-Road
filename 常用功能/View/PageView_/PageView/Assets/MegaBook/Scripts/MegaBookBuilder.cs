...
public class MegaBookBuilder : MonoBehaviour
{
	...
	{
		page = val;
		page = Mathf.Clamp(page, 0.0f, (float)NumPages);
		if ( force )
		{
			turnspd = 0.0f;
			Flip = page;
		    ...
		    for ( int i = 0; i < pageparams.Count; i++ )
		    {
			    ...
			    if ( page.pageobj == null )
			    {
				    if ( page.usebackground && (page.background || background ) )	//&& page.usebackground )
				    {
					    ...
					    page.madefront = MakePage(bg, page.front, rect, false);
				    }
				    ...
				    if ( page.usebackground1 && (page.background1 || background1) )	//&& page.usebackground1 )
				    {
					    ...
					    page.madeback = MakePage(bg, page.back, rect, true);
				    }
			    }
		    }
        }
	...
	Texture2D MakePage(Texture2D src, Texture2D image, Rect area, bool back)
	{
		if ( src )
		{
		...
		if ( width > 0.0f && length > 0.0f )
		{
			...
			for ( int iy = 0; iy < lengthsegs; iy++ )
			{
				for ( int ix = 0; ix < widthsegs; ix++ )
				{
					...
					{
						kv = grid[iy, ix];
						MakeQuad1(tris, kv, kv + 1, grid[iy + 1, ix + 1], grid[iy + 1, ix]);
					}
				}
			}
		}
	...
	void Update()
	{
		...
		//float alpha = (int)(Flip) / (float)page_count;
		//float alpha = Flip / (float)NumPages;
		//BottomAngle = Mathf.Lerp(0.0f, -180.0f, alpha);	//(Flip / (page_count * 14.0f)) * -180.0f;
		//BottomAngle = (Flip / (page_count * 14.0f)) * -BottomMaxAngle;	//.0f;
		if ( changespineangle )
		{
			float alpha = Flip / (float)NumPages;
		...
		//Vector3 rot = Vector3.zero;	//transform.localRotation.eulerAngles;
		//rot.z = -BottomAngle;
